...
AC_DEFUN([RAIM_INIT_MACH], [
	RAIM_ARG_VAR([MACH], [target machine])
	RAIM_SUPPORT_MACH([msim loongson3a zynq pc generic unknown])
	...
	AS_CASE([$MACH],
		...
		[riscv-virt], [
			AS_VAR_SET([__enable_bootloader], [yes])
			AS_VAR_SET([__with_cpus], [1])
            AS_VAR_SET([__with_ram_physbase], [0x80000000])
            AS_VAR_SET([__with_kern_start], [0x82000000])
		],
		[generic], [],
		[unknown], [],
		[msim], [
			 RAIM_SUBST_MULTILINE([MSIM_CONF_CPULIST],
			...
			RAIM_SUBST_MULTILINE(MSIM_CONF_MBLIST,
				`for (( i=0; $i<$with_cpus; i=$i+1 )); do \
